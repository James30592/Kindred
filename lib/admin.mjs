...
export class BatchUserCreator {
  ...
  _createUser(num){
    ...
    const userObj = {
      ...
      categoryTypesAnswers: BatchUserCreator._getRandAnswers(this.categoryInfo)
    };
  ...
  static _getRandAnswers(dbCategoryTypes) {
    let answers = [];

    dbCategoryTypes.forEach((dbCatType) => {
      answers.push({
        categoryType: dbCatType.name,
        categoriesAnswers: []
      });

      dbCatType.categories.forEach((dbCategory) => {
        answers.at(-1).categoriesAnswers.push({
          category: dbCategory.name,
          answers: []
        });

        dbCategory.questions.forEach((dbQuestion) => {
          const thisQId = dbQuestion._id;
          const [ansId, ansPerc] = BatchUserCreator._getRandAnsIdAndPerc(
            dbQuestion.possAnswers.length);
          ...
          answers.at(-1).categoriesAnswers.at(-1).answers.push(thisAnswer);
        });
      })
    })
