...
import { MY_CITIES } from "../adminFiles/myCities.mjs";
import { MY_NAMES } from "../adminFiles/myNames.mjs";
...
export class BatchUserCreator {
  ...
  static #LOCS_FILE = "adminFiles/locs.txt";
  static #AUTO_QS = "adminFiles/autoQs/";
  static #NUM_ANSWERS = 100;
  ...
  async #createRandAnswers(userId) {
    const createAnswersList = (categoryQs) => {
  ...
  async #createLocsFile() {
    ...
    for (let city of BatchUserCreator.#ALL_LOCS) {
      const url = `https://maps.googleapis.com/maps/api/geocode/json?address=${city}&key=${process.env.GOOGLE}`;
  ...
  static async #createUser(num) {
    ...
    const userObj = {
      email: `${name}@auto${num}.com`,
      profileName: name,
      location: BatchUserCreator.#getRandLoc(),
      isAdmin: false
    };

    const newUser = await User.register(userObj, name);
    return newUser._id;
  }
