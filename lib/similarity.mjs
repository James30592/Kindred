...
export class KindredList {
  ...
  constructor(mainUser, maxListLen = null) {
    ...
    this.basedOnCategoryAnswers = new dbHelpers.RelevantAnswers();
    this.relevantUsers = [];
  ...
  _getUserAnswersCatInfo(categoryInfo, allRelevantAnswers, mainUserId) {
    const args = {
      allRelevantAnswers: allRelevantAnswers, 
      userId: mainUserId
    };
...
class UserSimCompForRecommendations extends UserSimComp {
  ...
  constructor(user, basedOnCategoryInfo = null, basedOnRelevantAnswers = [], 
    recForCategoryInfo = null, recForRelevantAnswers = []) {

    super(user, basedOnCategoryInfo, basedOnRelevantAnswers);
    ...
    this.recForCategoryInfo = this._initRecForCatInfo(user, recForCategoryInfo, 
      recForRelevantAnswers);
  }
  
  // Gets this comp users answers to each result category and stores in 
  // a category info object.
  _initRecForCatInfo(user, recForCategoryInfo, recForRelevantAnswers) {
    const args = {
      allRelevantAnswers: recForRelevantAnswers, 
      userId: user._id
    };
...
const getUserAnswers = (catTypeName, categoryName, args) => {
  const userResultCatAnswers = args.allRelevantAnswers.getUserAnswers(
