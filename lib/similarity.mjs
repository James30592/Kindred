...
class UserSimComp extends CategoryInfo {
  ...
  updateSimInfos(user1, user2, relevantAnswers) {
...
export class KindredList {
  ...
  constructor(categoryInfo = null, maxListLen = null){
    this.categoryInfo = _getCatInfoWithUserAnswers(categoryInfo);
    this.simRatingList = [];
  ...
  //   
  _getCatInfoWithUserAnswers(categoryInfo) {
    ...
    for (let categoryWithType in allCategoriesWithTypes) {
      ...
      newCatInfo.checkAndAddCategoryWithType(catTypeName, categoryName, null)
    };
