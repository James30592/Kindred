...
class UserSimComp extends CategoryInfo {
  ...
  _getCategoryAnswers(categoryTypeName, categoryName) {
  ...
  updateSimInfos(mainUserAllAnswers) {
    for (let categoryTypeName in this.catTypes) {
      for (let categoryName in this.catTypes[categoryTypeName].categories) {
        const mainUserAnswers = mainUserAllAnswers.catTypes[categoryTypeName].categories[categoryName].answers;
        const compUserAnswers = _getCategoryAnswers(categoryTypeName, categoryName);
...
export class KindredList {
  ...
  findKindred() {
    // Create UserSimComps.
    this.relevantUsers.forEach(compUser => {
  ...
  // Initialise the relevant answer, relevant users and categoryInfo for this KindredList.
  async initKindredList(categoryInfo) {
    this.categoryInfo.catTypes = structuredClone(categoryInfo.catTypes);

    // Get all relevant categoryAnswers by all users for these categories.
    await this.relevantAnswers.initRelevantAnswers(categoryInfo);
    ...
    this.mainUserAnswers = _getUserAnswersCatInfo(categoryInfo, 
      this.relevantAnswers, this.mainUser._id);

    // Find any users who have answers in relevantAnswers.
    const findCriteria = {$or: []};
    const relevantUserIds = this.relevantAnswers.getAllUniqueUserIds();
    for (let userId of relevantUserIds) {
      findCriteria.$or.push({_id: userId});
    };
    this.relevantUsers = await models.User.find(findCriteria).exec();
  }

  // Creates a category info object with the user's answers, given an array of 
  // categoryAnswersLists also.
  _getUserAnswersCatInfo(categoryInfo, allRelevantAnswers, mainUserId) {
  ...
  _createSimComp(compUser) {
    const thisSimComp = new UserSimComp(compUser, this.categoryInfo, 
      this.relevantAnswers.allRelAnswers);
...
class UserSimCompForRecommendations extends UserSimComp {
  ...
  constructor(user, categoryInfo = null, relevantAnswers, recForCategoryInfo = null) {
    super(user, categoryInfo, relevantAnswers);
