...
export class ServerState {
    ...
    async init() {
        ...
        this.#getSelectableRecForCategories();
    }

    // Creates an array of just category names, for all the categories that are 
    // recommendable.
    #getSelectableRecForCategories() {
        const allUniqueCatsAndTypes = this.allCategoryInfo.getAllCategories(true);
        const recCatsAndTypes = allUniqueCatsAndTypes.filter(catAndType => {
            return catAndType?.isRecommendable;
        });

        this.selectableRecForCategories = recCatsAndTypes.map(catAndType => {
            return catAndType.category;
        });
    }
    ...
    async #initAllCategories() {
        ...
        this.allCategories.sort((a, b) => a.idx - b.idx);
        this.allCategoryInfo = new CategoryInfo(null, this.allCategories);
    }
