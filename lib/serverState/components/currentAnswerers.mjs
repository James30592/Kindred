import { DBCategoryAnswersList } from "../../dbHelpers/categoryQuestionOrAnswersList/sub-classes/categoryAnswersList.mjs";
...
export class CurrAnswerers {
    ...
    async #addUser(userId, catTypeName, categoryName) {
        const answersList = new DBCategoryAnswersList();
        await answersList.initAndCreateIfNeeded(catTypeName, categoryName, userId);

        const thisUser = await User.findOne({_id: userId}).exec();
    ...
    async #updateUserCategory(userId, catTypeName, categoryName) {
        const newAnswersList = new DBCategoryAnswersList();
