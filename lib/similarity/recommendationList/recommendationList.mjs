...
import { clamp } from "../../../src/sharedJs/utils.mjs";
...
export class RecommendationList extends KindredList {
  ...
  static #SIM_THRESHOLD = 6.5;

  constructor(mainUser, simThreshold) {
    super(mainUser);
    this.#simThreshold = simThreshold ?? RecommendationList.#SIM_THRESHOLD;
  }
  ...
  // Given a list of compUsers and a similarity threshold, populates this 
  // kindredlist with a KindredListForRecommendations object for all users
  ...
  getRecommendations() {
    this.simRatingList.forEach(simComp => {
      ...
      const thisScoreDiff = simComp.simInfo.scoreDiff;
      ...
      for (let categoryAndType of resultCategoriesAndTypes) {
        ...
        for (let answer of nonSkipCompUserAnswers) {
          ...
          const adjustedScore = clamp(answer.answerVal + 
            thisScoreDiff, 0, 10);
