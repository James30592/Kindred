...
export class RecommendationList extends similarity.KindredList {
  ...
  constructor(mainUser, simThreshold = 75) {
    ...
    this.resultCategoryAnswers = new dbHelpers.RelevantAnswers();
    ...
    this.resultCategoryQuestions = new dbHelpers.RelevantQuestions();
  }
  ...
  _initUserAnsweredQsInfo() {    
    const args = {
      resultRelevantAnswers: this.resultCategoryAnswers, 
    ...
    const getUserResultQIds = (catTypeName, categoryName, args) => {
      const userResultCatAnswers = args.resultRelevantAnswers.getUserAnswers(
  ...
  // the list.
  async _finaliseRecommendList(dbCatTypes) {
