...
export class CategoryAnswersList extends CategoryQuestionOrAnswersList {
  ...
  // Updates an answer in an answers list for a given new answer (does not call 
  // save on the DB doc).
  updateAnswer(newAnswer) {
    const ansIndex = this.item.answers.findIndex(ans => {
      return ans.questionId === newAnswer.questionId
    });
    
    this.item.answers.splice(ansIndex, 1, newAnswer);
  }
  ...
  // Adds an array of new answers to this answers DB doc (does not call save on 
  // the DB doc).
  addAnswers(newAnswers) {
    this.item.answers.push(...newAnswers);
  }
}
